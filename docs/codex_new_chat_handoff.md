...
- `python src/integration_debt_sweep.py --out docs/integration_debt_report.md
  --json-out src/integration_debt_report.json --format text` -> warn, 4
  warnings, 6 findings.
...
- Fundstrat web and video transcript handling is compact/public plus
  private-vault/raw: raw transcripts go only to the private source vault named
  by `INVESTING_OS_SOURCE_VAULT`; public repo state keeps metadata, hashes,
  short synthesis, and compact derived rows only.
- The public transcript index exists but currently has no registered transcript
  pack entries.
