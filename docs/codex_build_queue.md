...
- The transcript-vault helper now exists, so the next useful slice is likely
  either wiring the late-evening Fundstrat web/transcript sweep into repo
  prompt/manifest docs or closing/reclassifying the queued transcript item if
  the helper fully satisfies the operating need.
...
- The refreshed integration-debt report is `warn` with 4 warnings and 6
  findings.
