# src/today_decide.py (C5 exact fragment)

**C5 EXACT FRAGMENT (authored in-chat 2026-06-10, written against the verbatim archived C1–C4 modules).** Write to `src/today_decide.py`. Self-contained on purpose: depends only on archived modules, ships its own scoped CSS/JS, and needs exactly ONE repo-eyes wiring step (see the wiring-notes sibling page).

```python
"""TODAY — DECIDE: the decision-first surface (C5).

One module, two halves:

* :func:`build_today_decide_payload` — pure data assembly. Pulls the ranked
...
* :func:`render_today_decide_html` — payload → a self-contained HTML section
...
* ACT / PASS / RECHECK rails copy disposition lines to the clipboard; a
  second tap copies ``UNDO <card_id>`` and visually reverts — undo is real,
  not cosmetic (the C6 spine accepts the UNDO verb; see Task-3 addendum).
* Honest absence everywhere: unreadable book → pace not computed; missing
  congruence → "not checked"; no dispositions yet → said plainly.
"""
...
def _goal_anchor(feed: dict[str, Any], goal: dict[str, Any], today_iso: str) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "pace_line": "(display-only) book value not readable from feed — pace not computed",
    ...
    pace_line = (
        f"(display-only) gap ${gap:,.0f} · {months} months to {goal['window_horizon']}"
        f" · ≈ ${per_month:,.0f}/month — pace never feeds ranking or urgency"
    )
    return {
...
# ---------------------------------------------------------------------------
# HTML renderer — scoped, self-contained, zero network
# ---------------------------------------------------------------------------
_CSS = """
<style>
...
def _render_card(card: dict[str, Any], rank: int) -> list[str]:
    ...
    h.append('<div class="td-row">evidence: '
             + " · ".join(f"{_esc(k)} {v:+.2f}" for k, v in groups.items()) + "</div>")
    if win.get("named_trigger"):
        h.append(f'<div class="td-row">trigger: {_esc(win["named_trigger"])}'
                 + (f' · deadline {_esc(win.get("deadline"))}' if win.get("deadline") else "") + "</div>")
    for reason in (win.get("reasons") or [])[:2]:
        h.append(f'<div class="td-row">• {_esc(reason)}</div>')
    ...
    if suggested:
        h.append(f'<div class="td-row">execute: {_esc(suggested.get("owner"))} {_esc(suggested.get("broker"))} '
                 f'{_esc(suggested.get("account"))} · {_esc(suggested.get("tax_flag"))} · {_esc(suggested.get("why"))}</div>')
    for leg in (execn.get("legs") or []):
        line = (f'sell ${leg.get("sell_usd", 0):,.0f} in {_esc(leg.get("owner"))} {_esc(leg.get("broker"))} '
                f'{_esc(leg.get("account"))} · {_esc(leg.get("tax_flag"))}')
        if leg.get("proceeds_constraint"):
            line += f' · ⚠ {_esc(leg["proceeds_constraint"])}'
        h.append(f'<div class="td-row">execute: {line}</div>')
    for ex in (execn.get("excluded") or []):
        h.append(f'<div class="td-row">excluded: {_esc(ex.get("account"))} — {_esc(ex.get("why_not"))}</div>')
    ...
    h.append(f'<div class="td-row">impact: {_esc(impact.get("band"))} · material: '
             f'{"yes" if impact.get("material") else "no"}</div>')
    for c in card.get("conflicts") or []:
        h.append(f'<div class="td-chip">SOURCE-CONFLICT — {_esc(c["with"])}: “{_esc(c["their_claim"])}” '
                 f'vs this card: {_esc(c["card_claim"])} · resolve before acting</div>')
    ...
    h.append(
        ...
        f'<button class="td-rail" data-card="{cid}" data-verb="PASS" data-copy="PASS {cid} — reason: " '
...
def render_today_decide_html(payload: dict[str, Any]) -> str:
    ...
    h.append(f'<h2>TODAY — DECIDE <span style="color:#94a3b8;font-size:12px">built {_esc(payload["built"])}</span></h2>')
    if ga.get("book_value") is not None:
        h.append(f'<div class="td-anchor">${ga["book_value"]:,.0f} → ${ga["fi_target"]:,.0f} '
                 f'· {ga["pct_to_target"]}% there</div>')
    else:
        h.append('<div class="td-anchor">book value: not readable — honest absence</div>')
    ...
    h.append('<div class="td-plan">plan: '
             ...
             + (f' · shortfall ${short:,.0f}' if isinstance(short, (int, float)) else '')
             + f' · positions as of {_esc(pl.get("positions_as_of"))}</div>')
    for g in payload["gates"]:
        color = _GATE_COLORS.get(g.get("state"), "#94a3b8")
        h.append(f'<span class="td-gate" style="border-color:{color};color:{color}">'
                 f'{_esc(g.get("symbol"))} {_esc(g.get("state"))} · {_esc(g.get("confirm_rule"))} '
    ...
    for c in backlog:
        h.append(f'<div class="td-row">{_esc(c["ticker"])} · {_esc(c["direction"])} · '
                 f'${float(c.get("dollars") or 0):,.0f} · p{c.get("priority")}</div>')
    h.append("</details>")
    cong = payload["congruence"]
    if cong.get("status") == "ok":
        for row in cong.get("rows") or []:
            flag = "\U0001f6a9 " if row.get("flagged") else ""
            h.append(f'<div class="td-cong">{flag}{_esc(row["insight_id"])} · {_esc(row["line"])}</div>')
    else:
        h.append(f'<div class="td-cong">congruence: not checked — {_esc(cong.get("reason", ""))}</div>')
