...
| T2 | Codex | Restore caps-based sizing block on BUY/ADD decision cards across HTML and JSX parity | `src/directive_recs.py`; `src/today_decide.py`; `src/conviction_cockpit_v6.jsx`; sizing/parity tests; `src/ARCHITECTURE.md` | PR#18 | 2026-06-12 21:40 ET |
| T7 | Codex | Stated-balance assertion for SnapTrade staged book validation | `src/snaptrade_book_refresh.py`; `src/snaptrade_positions_import.py`; `src/build_positions_cache.py`; reconciliation tests | PR#16 | 2026-06-12 21:22 ET |
| T4 | Codex | Deepdive evidence battery: multi-day OI build plus 10-day dark-pool blocks | `src/deepdive_runner.py`; UW response adapters/tests; Notion rows `36ec5031-4bb6-814b` and `36ec5031-4bb6-81dd` | PR#17 | 2026-06-12 21:31 ET |
| T3 | Codex | Integration-debt sweep and Sunday Weekly Pilot Run hook | `src/integration_debt_sweep.py`; `docs/integration_debt_report.md`; weekly pilot prompt; dashboard warning count | CLAIMED | 2026-06-12 20:35 ET |
| T5 | Codex | Outcome-pattern loop for Trade Outcomes plus Decisions Log exports | `src/outcome_patterns.py`; Sunday Deep Synthesis prompt; outcome/decision export tests | CLAIMED | 2026-06-12 20:35 ET |
...
| T8 | Codex | Account semantics absorb for executable tax_status and account flags | `src/account_rules.json`; SnapTrade profile/semantics path; execution-plan placement tests | CLAIMED | 2026-06-12 20:35 ET |
| T9 | Codex | Held-decisions strip for operator-parked decision packets with review-by triggers | `src/held_decisions.json`; `src/held_decisions.py`; `src/test_held_decisions.py`; `src/cockpit_html_gen.py`; `src/trigger_registry.json`; `docs/codex_tasks/held_decisions_strip.md` | CLAIMED | 2026-06-13 00:00 ET |
...
| R1 | Claude Code | Research backlog triage: kill/uncertain/relevant split vs repo truth (positions/theses/triggers) | `docs/research_dossiers/TRIAGE_2026-06-13.md` | PUSHED · PR pending (no gh) | 2026-06-13 01:45 ET |
| R2 | Claude Code | Research feed fix (`--from-notion` intake) + ghost-routine audit/guard for off-hours-research-queue & top-prospects-auto-research | `src/research_queue_intake.py`; `src/test_research_queue_intake.py`; new overdue-guard test; `src/codex_routines/off_hours_research.md` | PUSHED · PR pending (no gh) | 2026-06-13 01:45 ET |
| R3 | Claude Code | Full research burn: evidence dossiers for every RELEVANT row + research block + INDEX | `docs/research_dossiers/*_2026-06-13.md`; `docs/research_dossiers/INDEX.md`; `src/research_queue.json` (research block only) | PUSHED · PR pending (no gh) | 2026-06-13 01:45 ET |
| R4 | Claude Code | Make tonight repeatable: 7:30 PM ET weekday delta-triage routine prompt + stamp-refresh + receipt | `src/codex_routines/off_hours_research.md` | PUSHED · PR pending (no gh) | 2026-06-13 01:45 ET |
| CLOUD-OFF-HOURS-2026-06-13 | Codex | Scheduled off-hours worker queue drain and verified Notion writeback | `src/cloud_routine_receipts.json`; live Notion Research Queue; routine-owned research artifacts if needed | MERGED | 2026-06-13 01:50 ET |
| CLOUD-TOP-PROSPECTS-2026-06-13 | Codex | Scheduled top-prospects auto-research constrained bridge run | `src/cloud_routine_receipts.json`; `src/top_prospects.json`; live Notion Research Queue if written | MERGED | 2026-06-13 20:48 ET |
| CLOUD-OFF-HOURS-2026-06-14 | Codex | Scheduled off-hours worker queue drain and verified Notion writeback | `src/cloud_routine_receipts.json`; live Notion Research Queue; routine-owned research artifacts if needed | MERGED | 2026-06-14 01:49 ET |
| CLOUD-DEEP-2026-06-14 | Codex | Scheduled deep synthesis trailing-30d review and verified Notion writeback | `src/cloud_routine_receipts.json`; live Notion Deep Synthesis page; monthly candidates only if written | MERGED | 2026-06-14 13:09 ET |
