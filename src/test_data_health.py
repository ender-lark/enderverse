...
def test_fresh_source_within_cadence():
    ...
    out = dh.assess(f, now=NOW, rates_path="/nonexistent/x.json")
...
def test_fs_inbox_absent_is_not_checked_never_all_clear():
    out = dh.assess(_feed(), now=NOW, rates_path="/nonexistent/x.json")
    it = [i for i in out["items"] if i["source"] == "fs_inbox"][0]
...
def test_empty_track_record_announces_but_never_blocks(tmp_path):
    ...
    out = dh.assess(_feed(fs_unread={"count": 0, "checked_at": "x"}), now=NOW, rates_path=rp)
...
def test_stale_gate_blocks():
    ...
    out = dh.assess(_feed(), gates=gates, now=NOW, rates_path="/nonexistent/x.json")
