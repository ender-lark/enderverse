...
| `decision_dossier_sync.py` + `decision_dossier_refresh.py` + `decision_dossier_coverage.py` + `decision_dossiers.py` + `decision_dossiers.json` | Live Theses-to-repo dossier sync, cached-evidence dynamic read refresh, Source Proof coverage audit, and runtime mirror. Adds card context only; stale price/timing reads render UNKNOWN and never change scoring/ranking. |
...
* `decision_dossier_coverage.py` runs during the full cockpit build after Today/Decide assembly and writes only `source_audits.decision_dossier_coverage`. It lists current action/material tickers that have no repo dossier row as coverage debt. Missing dossiers do not create `data_health` blockers, alerts, score changes, gates, sizing changes, or trade posture.
...
* `cloud_routine_commit.DEFAULT_ALLOWED_PATHS` now allows `dispositions.jsonl`, `timing_gates.json`, `prediction_signals.json`, `uw_endpoint_results.json`, `source_rates.json`, `options_decay_audit.json`, and `options_shadow_log.jsonl` to be committed by scheduled routines.
...
* The dashboard refresh sequence runs `source_call_tracker.py --hit-rates`
  before feed builds, regenerating `src/source_rates.json` from real
...
* `options_decay_audit.py` is a routine-side risk monitor outside the pure feed
  engine. It reads `account_positions.json` plus optional fresh
  `options_chain_cache.json`, writes review-only `options_decay_audit.json`,
  may send Pushover review prompts, and keeps stale/missing chain coverage
  `not_checked`.
