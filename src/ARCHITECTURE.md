...
2. **`actions_read` is additive, not generative.** It ranks ⑦+⑧ output; it doesn't manufacture actions. → on a genuinely quiet day, an empty actions lane is *honest*, not broken. The enrichment path is the reserved `synthesis_actions` seam (Synthesis Brain, unbuilt) — **not** ad-hoc invention.
